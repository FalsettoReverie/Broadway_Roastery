(function( $ ) {
	let calenderTpl = `
		<div id="calTitle">
			...
			<button type="button" class="month-mover next" style="display: block">
				<svg fill="#FFFFFF" height="30" viewBox="0 0 24 24" width="30" xmlns="http://www.w3.org/2000/svg">
	...
	`;
	let weekDaysFromSunday = '<div>S</div><div>M</div><div>T</div><div>W</div><div>T</div><div>F</div><div>S</div>';
	let weekDaysFromMonday = '<div>M</div><div>T</div><div>W</div><div>T</div><div>F</div><div>S</div><div>S</div>';
		...
		function showEvent(event){
			if(event && event !== null && event !== undefined){
				...
				if(dataContainer.firstChild)
				{
                    ...
                    const eventList = dataContainer.appendChild(document.createElement('ul'));
					for( let key in event)
					{
						if(key==="date")
						{
						 ...
						 listnode.appendChild(document.createTextNode(key.substr(0,1).toUpperCase()+key.substr(1)+ ": "+ event[key]));
